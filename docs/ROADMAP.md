...
3. **Governed Proposer Capabilities:** Without any hardcoded PO numbers, amounts, or alert text in agent/server code, queries like *"Why is this invoice blocked?"* propose `SCROLL_TO_ELEMENT` to the `Price Variance Alert`, while *"How do I request an override for this variance?"* proposes `CLICK_ELEMENT` on `Request Variance Manager Approval`.
4. **DOM Observation Sanitation:** `apps/browser-extension/src/security/sanitize.ts` indexes `[role='alert']` and `[role='status']` into `SafeControl[]`.
5. **Direct View & Scenario Routing:** In `tests/fixtures/sap-s4hana.html`, URL parameters (`?scenario=missing-supplier`, `?scenario=price-variance`, `?view=create-po`, `?view=create-invoice`) allow direct access to views and test scenarios.
6. **Live Verification:** All 8 scenarios in `tests/fixtures/verify.html` pass with real browser CDP automation (`tests/fixtures/run-live-verify.mjs`), validating preflight re-observation, non-blind execution, and audit trail integrity.
