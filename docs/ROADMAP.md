...
4. **Finance Workspace & Launchpad Integration:** Added the dedicated `Finance & General Ledger Accounting` workspace (`#view-finance`) accessible from the Finance tab (`#tab-finance`) and direct route (`?view=finance`). It features interactive tiles for `Automatic Account Determination (OBYC)`, `Supplier Invoices (MIRO)` (flagged with 1 Error Blocked), `Material Valuation (MM03)`, and `Valuation Classes (OMSK)`. Additionally, the launchpad (`My Home`) displays the blocked `Supplier Invoices` tile and an `Automatic Account Determination (OBYC)` tile with direct modal triggers so users can initiate and review OBYC configuration and clearing accounts from both My Home and Finance.
5. **Governed Proposer Capabilities:** Without any hardcoded PO numbers, amounts, or alert text in agent/server code, queries like *"Why is this invoice blocked?"* propose `SCROLL_TO_ELEMENT` to the `Price Variance Alert`, while queries like *"Review OBYC Configuration"* propose `CLICK_ELEMENT` on `Review OBYC Configuration`.
6. **DOM Observation Sanitation:** `apps/browser-extension/src/security/sanitize.ts` indexes `[role='alert']` and `[role='status']` into `SafeControl[]`.
7. **Direct View & Scenario Routing:** In `tests/fixtures/sap-s4hana.html` and `tests/fixtures/sap-po-invoice.html`, URL parameters (`?scenario=missing-supplier`, `?scenario=price-variance`, `?scenario=account-determination`, `?view=create-po`, `?view=create-invoice`, `?view=finance`) allow direct access to views and test scenarios.
8. **Live Verification:** All scenarios pass automated and browser tests, validating preflight re-observation, non-blind execution, and audit trail integrity.
